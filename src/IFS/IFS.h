...
struct FileStat {
	...
	FileStat(char* namebuf, uint16_t bufsize)
	{
		name.buffer = namebuf;
		name.size = bufsize;
	}
...
public:
     ...
     *  to track other information. It releases memory when closedir() is
     *  called.
     */
     ...
     *  @param stat structure to return information in, may be null to do
     *  a simple file existence check
     *  @retval int error code
     */
